...
import { Scanner, type Token, type TokenKind } from './scanner.js'
import type { ParsedNote, ParsedTune, DocumentBlock } from './types.js'
...
const LY_DISPLAY: Record<string, string> = {
 ...
 */
export function enharmonicSpelling(pitchClass: number, diatonicClass: number): string {
...
export function extractKey(src: string): string {
  ...
  for (let i = 0; i < tokens.length - 2; i++) {
    if (tokens[i]!.kind === 'command' && tokens[i]!.value === 'key') {
      ...
      if (!noteToken || !modeToken) continue
...
export function extractTimeSig(src: string): string {
  const sc = new Scanner(src)
  const tokens = sc.tokenize()
  for (let i = 0; i < tokens.length - 2; i++) {
...
function extractHeaderField(src: string, fieldName: string): string | undefined {
...
export function extractComposer(src: string): string | undefined {
 ...
 */
export function extractPaper(src: string): { raggedLast?: boolean; indent?: number; font?: string } {
  const result: { raggedLast?: boolean; indent?: number; font?: string } = {}
...
interface VarDef {
  body: string          // raw text inside { }
  startNote: string     // e.g. "c''" for \relative c'' — empty string if bare \relative or absolute
  isRelative: boolean   // true when \relative was present (even with no start note)
  isChordMode?: boolean // true when \chordmode { ... }
}
 ...
 */
export function extractVariables(src: string): Map<string, VarDef> {
  ...
  while (i < src.length) {
    ...
    if (!isIdentStart(src[i]!)) { i++; continue }
    const nameStart = i
    ...
    if (src.startsWith('\\relative', i) || src.startsWith('\\fixed', i)) {
      ...
      if (i < src.length && src[i]! >= 'a' && src[i]! <= 'g') {
        const snStart = i
        while (i < src.length && (isAlpha(src[i]!) || src[i] === "'" || src[i] === ',')) i++
 ...
 */
export function findMelodyVarName(src: string, vars: Map<string, VarDef>): string | undefined {
  // Scan for \new Staff { \varname }
  const sc = new Scanner(src)
  const tokens = sc.tokenize()
  for (let i = 0; i < tokens.length - 3; i++) {
    ...
    if (t.kind === 'command' && t.value === 'new') {
      ...
      if (ctx?.kind === 'word' && ctx.value === 'Staff') {
        ...
        while (j < tokens.length && tokens[j]!.kind !== 'open') j++
 ...
 */
export function parseDuration(durStr: string, prevDuration: number): number {
  if (durStr === '') return prevDuration
  let s = durStr
  const dots = countLeadingFromRight(s, '.')
  s = s.slice(0, s.length - dots)
  if (s === '') return prevDuration
  const base = 4 / parseInt(s, 10)  // e.g. "8" → 0.5 quarter notes
  let dur = base
  for (let i = 0; i < dots; i++) dur += base / Math.pow(2, i + 1)
  return dur
}

function countLeadingFromRight(s: string, ch: string): number {
  let n = 0
...
const CHORD_ROOT_DISPLAY: Record<string, string> = {
  c: 'C', cs: 'C#', cf: 'Cb',
  d: 'D', ds: 'D#', df: 'Db',
  e: 'E', ef: 'Eb', es: 'E#',
  f: 'F', fs: 'F#', ff: 'Fb',
 ...
 */
function chordQualitySuffix(mod: string): string {
...
export function parseChordMode(body: string, vars: Map<string, VarDef>, defaultDuration = 4): ChordName[] {
  ...
  function collect(): void {
    while (i < tlen) {
      ...
      if (t.kind === 'command' && t.value === 'repeat') {
        ...
        // \alternative { { e1 } { e2 } }
        let endings: ChordName[][] = []
        // look ahead for \alternative
        let j = i
        while (j < tlen && tokens[j]!.kind !== 'command' && tokens[j]!.kind !== 'eof') j++
        if (j < tlen && tokens[j]!.kind === 'command' && tokens[j]!.value === 'alternative') {
          ...
          // each { ... } is one ending
          while (i < tlen) {
            while (i < tlen && tokens[i]!.kind !== 'open' && tokens[i]!.kind !== 'close') i++
            if (tokens[i]?.kind === 'close') { i++; break }  // outer }
            i++ // consume inner {
        ...
        if (endings.length === 0) {
          for (let r = 0; r < repeatCount; r++) chords.push(...bodyChords)
        } else {
          const plainTimes = Math.max(0, repeatCount - endings.length)
          for (let r = 0; r < plainTimes; r++) chords.push(...bodyChords)
          for (const ending of endings) { chords.push(...bodyChords); chords.push(...ending) }
        }
        continue
      }
      ...
      if (t.kind === 'command' && vars.has(t.value)) {
        ...
        if (sub.length > 0) { chords.push(...sub); prevDur = sub[sub.length - 1]!.duration }
        continue
      }
      ...
      if (t.kind === 'note') {
        ...
        // ends after the last non-whitespace char of the note+duration.  Use t.pos as start.
        {
          ...
          while (k < blen && /[a-zA-Z]/.test(body[k]!)) k++
          // skip octave marks (' and ,)
          while (k < blen && (body[k] === "'" || body[k] === ',')) k++
          // skip duration digits and dots
          while (k < blen && /[\d.]/.test(body[k]!)) k++
          // skip fingering (-N or _N)
          if (k < blen && (body[k] === '-' || body[k] === '_') && /\d/.test(body[k + 1] ?? '')) {
            k += 2; while (k < blen && /\d/.test(body[k]!)) k++
          }
          // skip whitespace
          while (k < blen && (body[k] === ' ' || body[k] === '\t')) k++
          ...
          if (k < blen && body[k] === ':') {
            ...
            const modStart = k
            while (k < blen && !/[\s|/\\{}><~,^_]/.test(body[k]!)) k++
      ...
      if (t.kind === 'rest') {
        i++
        if (t.value) prevDur = parseDuration(t.value, prevDur)
...
interface RelCtx {
  prevDiatonic: number   // diatonic pitch index: octave*7 + diatonicClass
  prevSemitones: number  // absolute semitones from C0
  prevDuration: number   // quarter-note units
  absolute?: boolean     // true when NOT inside \relative — use direct octave modifiers
}
...
function resolveRelative(
  lyName: string,   // e.g. 'g', 'fs', 'bf'
  octaveStr: string, // e.g. "''", ",", ""
  ctx: RelCtx,
): { semitones: number; diatonic: number; octave: number; pitchClass: number } {
...
function parseStartNote(startNote: string, isRelative: boolean = false): RelCtx {
  if (!startNote) {
    ...
    if (isRelative) return { prevDiatonic: 3 * 7 + 3, prevSemitones: 3 * 12 + 5, prevDuration: 1 }
...
class TokenCursor {
  ...
  get done(): boolean { return this.pos >= this.tokens.length || this.tokens[this.pos]!.kind === 'eof' }
  ...
  skip(kind: TokenKind, value?: string): boolean {
    if (this.is(kind, value)) { this.advance(); return true }
    return false
  }

  skipCommand(name: string): boolean {
    if (this.isCommand(name)) { this.advance(); return true }
    return false
  }
}
 ...
 */
function parseMusicSeq(cur: TokenCursor, ctx: RelCtx, vars: Map<string, VarDef>): ParsedNote[] {
  ...
  while (!cur.done && !cur.is('close') && !cur.is('close', '>>')) {
    ...
    // Bar check — skip
    if (t.kind === 'barcheck') { cur.advance(); continue }

    // Tie: mark the note just pushed as tieStart; next note will get tieEnd
    if (t.kind === 'tie') {
      cur.advance()
      if (notes.length > 0) notes[notes.length - 1]!.tieStart = true
      pendingTieEnd = true
      continue
    }
    ...
    if (t.kind === 'slur_open') {
      cur.advance()
      if (notes.length > 0) notes[notes.length - 1]!.slurStart = true
      continue
    }
    if (t.kind === 'slur_close') {
      cur.advance()
      if (notes.length > 0) notes[notes.length - 1]!.slurEnd = true
      continue
    }
    ...
    if (t.kind === 'open' && t.value === '<<') {
      ...
      while (!cur.done && !(cur.is('close') && cur.peek().value === '>>')) cur.advance()
      cur.advance() // >>
      continue
    }
    ...
    if (t.kind === 'note') {
      ...
      let octave: number, pitchClass: number
      const base = lyName[0]!
      ...
      const note: ParsedNote = { noteName, octave, pitchClass, duration: ctx.prevDuration, isRest: false }
      if (fingeringStr) {
      ...
      if (pendingTieEnd) { note.tieEnd = true; pendingTieEnd = false }
      notes.push(note)
      continue
    }
    ...
    if (t.kind === 'chord_open') {
      ...
      while (!cur.done && !cur.is('chord_close')) {
        ...
        if (ct.kind === 'note') {
          cur.advance()
          const [lyName, octStr, durStr] = ct.value.split('|') as [string, string, string]
          void durStr
      ...
      if (chordNotes.length > 0) {
        const top = chordNotes.reduce((a, b) =>
          (a.octave * 12 + a.pitchClass) >= (b.octave * 12 + b.pitchClass) ? a : b
        )
        ...
        const chordNote: ParsedNote = { ...top, duration: ctx.prevDuration }
        if (others.length > 0) chordNote.chordNotes = others
        if (pendingTieEnd) { chordNote.tieEnd = true; pendingTieEnd = false }
    ...
    if (t.kind === 'command') {
      ...
      if (cmd === 'repeat') {
        ...
        let endings: ParsedNote[][] = []
        const endingVoltaNumbers: number[] = []
          ...
          while (cur.is('open') || cur.isCommand('volta')) {
            ...
            if (cur.isCommand('volta')) {
              ...
              if (cur.is('number')) {
                ...
                while (cur.is('number')) cur.advance()  // skip remaining numbers in list
              }
            }
      ...
      if (cmd === 'appoggiatura' || cmd === 'acciaccatura' || cmd === 'grace' || cmd === 'slashedGrace') {
        ...
        if (cur.is('open')) {
          ...
          const singleTokens: Token[] = [noteTok, { kind: 'eof', value: '', pos: -1 }]
        ...
        const gType = cmd as ParsedNote['graceType']
      ...
      if (cmd === 'mark') {
        ...
        while (cur.isCommand('markup') || cur.isCommand('box')) cur.advance()
        if (cur.is('string')) markText = cur.advance().value
        if (markText) {
      ...
      if (cmd === 'tempo') {
        ...
        let beatDur = 0
        let bpm = 0
        if (cur.is('string')) text = cur.advance().value
        if (cur.is('number')) {
          beatDur = parseInt(cur.advance().value, 10)
          cur.skip('equals')
          if (cur.is('number')) bpm = parseInt(cur.advance().value, 10)
        }
        // Remaining words/numbers (e.g. markup text) — skip
        while (!cur.done && (cur.is('word') || cur.is('number') || cur.is('slash'))) cur.advance()
        if (text || bpm) {
      ...
      if (cmd === 'time') {
        let n = '', d = ''
        if (cur.is('number')) n = cur.advance().value
        cur.skip('slash')
        if (cur.is('number')) d = cur.advance().value
        if (n && d) {
      ...
      if (cmd === 'transpose') {
        ...
        function tokPitch(v: string): string {
        ...
        notes.push(...bodyNotes.map(n => {
          if (n.isRest || n.pitchClass < 0) return n
          if (isPureOctave) return { ...n, octave: n.octave + octaveShift }
 ...
 */
export function parseLy(src: string): ParsedTune {
  ...
  if (!melodyName) {
    ...
    const raw    = parseMusicSeq(cur, ctx, vars)
    if (raw.length > 0) return buildTune({ title, composer, key, timeSig, paper }, raw)
 ...
 */
function extractBareMusicBlock(src: string): string | null {
  let i = 0
...
// ── Shared tune-building helper ───────────────────────────────────────────────

interface TuneMeta {
  title?: string; composer?: string; key: string; timeSig: string
  paper: { raggedLast?: boolean; indent?: number; font?: string }
}
 ...
 */
function buildTune(meta: TuneMeta, raw: ParsedNote[]): ParsedTune {
  const { title, composer, key, timeSig, paper } = meta
  const notes: ParsedNote[] = []
  const systemBreaks: number[] = []
  const repeatRegions: Array<{ start: number; end: number }> = []
  const voltaRegions:  Array<{ start: number; end: number; volta: number }> = []
  const rehearsalMarks: Array<{ noteIndex: number; text: string }> = []
  const tempoMarks: Array<{ noteIndex: number; text?: string; bpm?: number; beatDuration?: number }> = []
  const pendingRepeatStarts: number[] = []
  let pendingVolta: { start: number; volta: number } | null = null
  ...
  for (const n of raw) {
    if (n.noteName === '\x00break') {
      ...
      if (start !== undefined) repeatRegions.push({ start, end: notes.length })
    } else if (n.noteName.startsWith('\x00volta') && n.noteName.endsWith('Start')) {
      const volta = parseInt(n.noteName.slice(6, -5), 10)
      if (pendingVolta) voltaRegions.push({ start: pendingVolta.start, end: notes.length, volta: pendingVolta.volta })
 ...
 */
function parseMarkupExpr(src: string, pos: number): {
  text: string; bold: boolean; italic: boolean; large: boolean;
  color?: string; code: boolean; end: number
} | null {
  const len = src.length
  let i = pos
  ...
  function ident(): string {
    const s = i
    while (i < len && ((src[i]! >= 'a' && src[i]! <= 'z') || (src[i]! >= 'A' && src[i]! <= 'Z') || src[i] === '-')) i++
  ...
  function colorArg(): string | undefined {
    if (src[i] === '"') return quoted()
    if (src[i] === '#') {
      i++
      if (src[i] === '(') {
        let d = 1; i++
        while (i < len && d > 0) { if (src[i] === '(') d++; else if (src[i] === ')') d--; i++ }
        return undefined // Scheme expr — skip
      }
      const s = i
      while (i < len && ((src[i]! >= 'a' && src[i]! <= 'z') || src[i] === '-')) i++
      return src.slice(s, i) || undefined
    }
    return undefined
  }

  let bold = false; let italic = false; let large = false
  let color: string | undefined; let code = false
  ...
  function mods(): void {
    ...
    while (i < len && src[i] === '\\') {
      ...
      else if (m === 'large' || m === 'larger')    large  = true
      else if (m === 'typewriter' || m === 'verbatim') code = true
      else if (m === 'with-color')               { color = colorArg(); ws() }
      // Unknown modifier — keep reading (it may prefix the text)
    }
  }

  function result(text: string): ReturnType<typeof parseMarkupExpr> {
    if (!text) return null
  ...
  function parseBracedContent(): string {
    ws()
  ...
  if (src[i] === '{') {
    ...
    if (src[i] === '{') {
      ...
      ws(); if (src[i] === '}') i++ // close inner {
      ws(); if (src[i] === '}') i++ // close outer {
      return result(text)
    }
 ...
 */
export function parseDocument(src: string): DocumentBlock[] {
  const clean = stripComments(src)
  const key = extractKey(clean)
  const timeSig = extractTimeSig(clean)
  const title = extractTitle(clean)
  ...
  function findMatchingClose(start: number): number {
    let depth = 1
    let j = start
    while (j < len && depth > 0) {
      if (clean[j] === '{') depth++
  ...
  while (i < len) {
    ...
    if (clean[i] === '\\') {
      ...
      // Read command name
      const nameStart = i
      while (i < len && ((clean[i]! >= 'a' && clean[i]! <= 'z') || (clean[i]! >= 'A' && clean[i]! <= 'Z'))) i++
      ...
      if (cmd === 'score' && depth === 0) {
        ...
        if (i < len && clean[i] === '{') {
          ...
          function extractStaffBody(body: string): string {
            ...
            for (let ti = 0; ti < toks.length - 2; ti++) {
              ...
              if (t.kind === 'command' && t.value === 'new') {
                ...
                if (nx?.kind === 'word' && (nx.value === 'Staff' || nx.value === 'Voice')) {
                  let tj = ti + 2
                  while (tj < toks.length && toks[tj]!.kind !== 'open') tj++
          ...
          function findChordNamesVarName(body: string): string | undefined {
            const toks = new Scanner(body).tokenize()
            for (let ti = 0; ti < toks.length - 1; ti++) {
              ...
              if (t.kind === 'command' && t.value === 'new') {
                ...
                if (nx?.kind === 'word' && nx.value === 'ChordNames') {
                  ...
                  let tj = ti + 2
                  while (tj < toks.length && toks[tj]!.kind !== 'command' && toks[tj]!.kind !== 'open') tj++
                  const ref = toks[tj]
          ...
          if (staffBody.trim()) {
            ...
            if (chordVarDef?.isChordMode) {
              ...
              if (chords.length > 0) tune.chordNames = chords
            }
