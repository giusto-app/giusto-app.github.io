...
export class Scanner {
    ...
    nextToken() {
        for (;;) {
            ...
            if (ch === '{') {
                this.advance();
                return { kind: 'open', value: '{', pos: start };
            }
            if (ch === '}') {
                this.advance();
                return { kind: 'close', value: '}', pos: start };
            }
            if (ch === '|') {
                this.advance();
                return { kind: 'barcheck', value: '|', pos: start };
            }
            if (ch === '~') {
                this.advance();
                return { kind: 'tie', value: '~', pos: start };
            }
            if (ch === '=') {
                this.advance();
                return { kind: 'equals', value: '=', pos: start };
            }
            if (ch === '/') {
                this.advance();
            ...
            if (ch === '<') {
                ...
                this.advance();
                return { kind: 'chord_open', value: '<', pos: start };
            }
            if (ch === '>') {
                if (this.peek(1) === '>') {
                    this.advance();
                ...
                this.advance();
                return { kind: 'chord_close', value: '>', pos: start };
            }
            // Octave modifiers (standalone — normally consumed inside note token)
            if (ch === "'") {
                this.advance();
                return { kind: 'note', value: `|'|`, pos: start };
            }
            // Quoted string
            if (ch === '"') {
                this.advance();
                return { kind: 'string', value: this.readQuotedString(), pos: start };
            }
            // Backslash command
            if (ch === '\\') {
                this.advance();
                if (!this.done && this.peek() === '(') {
                    this.advance();
                    return { kind: 'slur_open', value: '\\(', pos: start };
                }
                if (!this.done && this.peek() === ')') {
                    this.advance();
                    return { kind: 'slur_close', value: '\\)', pos: start };
                }
            ...
            if (ch === '^') {
                this.advance();
                return { kind: 'markup_above', value: '^', pos: start };
            }
            if (ch === '_') {
                ...
                if (!isDigit(this.peek(1))) {
                    this.advance();
                    return { kind: 'markup_below', value: '_', pos: start };
                }
            }
