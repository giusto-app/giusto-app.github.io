...
export default function TabBar({ activeTab, onChange }: TabBarProps) {
  return (
    <nav className="flex bg-gray-900 border-t border-gray-800 pb-[env(safe-area-inset-bottom)]">
      {TABS.map(({ id, label, Icon }) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(id)}
            className={[
              'flex-1 flex flex-col items-center gap-1 py-3 text-xs font-medium transition-colors active:scale-95 touch-none',
...
function PracticeIcon({ active }: { active: boolean }) {
  return (
    <svg width={22} height={22} viewBox="0 0 24 24" fill="none"
      ...
      <circle cx="12" cy="12" r="3" fill={active ? '#34d399' : 'currentColor'} stroke="none" />
    </svg>
  )
}
