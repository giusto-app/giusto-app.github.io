...
export default function StartButton({ listeningState, onStart, onStop }: StartButtonProps) {
  if (listeningState === 'listening') {
    return (
      <button
        onClick={onStop}
        className="flex items-center gap-2 px-8 py-4 neu-btn text-[color:var(--neu-fg)] rounded-full"
      >
  ...
  return (
    <button
      onClick={onStart}
      className="flex items-center gap-3 px-8 py-4 neu-btn text-[color:var(--neu-fg)] rounded-full"
    >
      <MicIcon />
      Start Listening
    </button>
